...
namespace PowerSeat통합검사기
{
    ...
    public partial class MainForm : Form, MyInterface
    {
        ...
        */
        ...
        private bool ProductInFlag = false;
        private int ProductOutCount = 0;
                                ...
                                if (PassButtonCheckFlag == false)
                                {
                                    if (RunningFlag == false)
                                    {
                                        ...
                                        if (led4.Value.AsBoolean == true)//제품이 있을 경우
                                        {
                                            ...
                                            else if ((label16.Text == "OK") && (xFlag == false))
                                            {
...
//#endif
            ...
            switch(SbrStep)
            {
                ...
                case 3:
                    ...
                    else
                    {
                        ...
                        {
                            if(IOPort.RetractorOnOff == true) IOPort.RetractorOnOff = false;
        ...
        /// <summary>
        /// 안티핀치 동작을 진행하도록 메시지를 전송한다.
        /// 안티핀치 기능은 리클라인 후진(BWD) 동작중 일정 압력을 받으면 이 기능을 동작 시킨다.
        /// 또한 리클라인 후진 중 이므로 안티핀치 동작을 하면 강제구동 메시지를 꺼 주어야 한다.
        /// </summary>
        public bool AntiPinch
        {
            set
            {
                if (comboBox3.SelectedItem.ToString() == "LH") 
                    CanReWrite.CanDataOutput(Data: value == true ? (byte)0x01 : (byte)0x00, Msg: OUT_CAN_LIST.RL_RelaxPSeat_AntipinchSet);
                else CanReWrite.CanDataOutput(Data: value == true ? (byte)0x01 : (byte)0x00, Msg: OUT_CAN_LIST.RR_RelaxPSeat_AntipinchSet);
            }
        }
        ...
        private bool InComingRun = false;
        
        private bool DeliveryPosMoveing()
        {
            if (SpecOutputFlag == false)
            {
            ...
            else
            {
                ...
                {
                    if (0.5F < AdData) DeliveryPosMoveFlag = true;
                }
                else
                {
        ...
        private void DeliveryResultDisplayt()
        {
            ...
            if (CheckItem.RelaxSeat == false)
            {
                if ((StrokeData < (mSpec.DeliveryPos.Recliner - mSpec.DeliveryPos.MinusRange)) ||
                    ((mSpec.DeliveryPos.Recliner + mSpec.DeliveryPos.PluseRange) < StrokeData))
                {
                    label5.Text = "납입위치 불량 입니다.";
                    label5.ForeColor = Color.Red;
                    return;
            ...
            {
                if ((StrokeData < (mSpec.DeliveryPos.RelaxStroke - mSpec.DeliveryPos.MinusRange)) ||
                    ((mSpec.DeliveryPos.RelaxStroke + mSpec.DeliveryPos.PluseRange) < StrokeData))
                {
                    label5.Text = "납입위치 불량 입니다.";
                    label5.ForeColor = Color.Red;
                    return;
