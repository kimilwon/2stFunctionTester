                ...
                if ((0 < buffer[0]) && (buffer[1] == READ_COMMAND) && (0 < buffer[2]))
                {
                    ...
                    unchecked //overflow 검출하지 않는다. 여산중 overflow가 발생하더라도 무시한다.
                    {
                        ...
                        if (crc16_high == buffer[Length - 2] && crc16_low == buffer[Length - 1])
                        {
                            ...
                            if (mControl.GetConfig.BattID == buffer[0])
                                Batt = Value;
                            //else if (mControl.GetConfig.PSwitchID == buffer[0])
                            else if (mControl.GetConfig.BuckleID == buffer[0])
                                Buckle = Value;
                            else if (mControl.GetConfig.HeaterID == buffer[0])
                                Curr = Value;

                            PanelMeterReadPos1++;
                            if (2 < PanelMeterReadPos1) PanelMeterReadPos1 = 0;
                            MessageDisplayFlag1 = false;
                        }
                    }
                    return;
        ...
        private void ADReadMessage(short Mode)
        {
            ...
            try
            {
                if (Mode == 0)
                {
                    ...
                    if ((PanelMeterReadPos1 < 0) || (2 < PanelMeterReadPos1)) PanelMeterReadPos1 = 0;
                    switch (PanelMeterReadPos1)
                    {
                        case 0:
                            ID = (byte)mControl.GetConfig.BattID;
                            break;
                        case 1:
                            ID = (byte)mControl.GetConfig.BuckleID;
                            break;
                        default:
                            ID = (byte)mControl.GetConfig.HeaterID;
                            break;
                    }
